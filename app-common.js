...
function requireAuth(onReady) {
  if (window.__TS_ISE_CONFIG_OK__ === false) {
    // firebase-config.js側で赤い警告バナーを表示済み。ここでは無限ローディングにしない。
    return;
  }
  showLoading("読み込み中…");
  const fallback = setTimeout(() => {
    showLoading("接続に時間がかかっています。通信環境をご確認ください（このまま数秒待っても変わらない場合はログイン画面から やり直してください）");
  }, 6000);
  try {
    auth.onAuthStateChanged(
      ...
      (err) => {
        ...
        showLoading("認証エラーが発生しました。ログイン画面に戻ります…");
